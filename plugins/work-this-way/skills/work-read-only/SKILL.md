...
Activate `READ ONLY`. Before any later operation, inspect its manifest traits. If it has `mutation`, do not call mutating tools and return `blocked`. Preserve the requested operation and its binding in the Working Object so a safe later card can consume the block.
